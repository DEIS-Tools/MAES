            ...
            if (currentCoordinate.x < nextCoordinate.x) {
                ...
                if (currentCoordinate.y < nextCoordinate.y) {
                   ...
                   // cc
                    ...
                    if ((currentCoordinateSlam[3] != open || bottomRightSlam[2] != open ||
                        topLeftSlam[1] != open || nextCoordinateSlam[0] != open) &&
                        CheckForUnseen(nextCoordinate + Vector2Int.left) && CheckForUnseen(currentCoordinate + Vector2Int.right))
                        {
                            if ( (CheckForUnseen(currentCoordinate) ||  CheckForUnseen(nextCoordinate) || (CheckIfAllSlamStatusesSolid(currentCoordinate + Vector2Int.right) &&
                                    CheckIfAllSlamStatusesSolid(nextCoordinate + Vector2Int.left))) && !(CheckIfAnyIsStatus(currentCoordinate, solid) || CheckIfAnyIsStatus(nextCoordinate, solid)))
                                {
                                }
                                    //Debug.Log($"Case xn\n" +
                                    //          $"Case cx\n" +
                    ...
                    if ((currentCoordinateSlam[1] != open || topRightSlam[0] != open ||
                        bottomLeftSlam[3] != open || nextCoordinateSlam[2] != open) &&
                        CheckForUnseen(nextCoordinate + Vector2Int.left) && CheckForUnseen(currentCoordinate + Vector2Int.right))
                        {
                            if ( (CheckForUnseen(currentCoordinate) ||  CheckForUnseen(nextCoordinate) || (CheckIfAllSlamStatusesSolid(nextCoordinate + Vector2Int.right) &&
                                  CheckIfAllSlamStatusesSolid(currentCoordinate + Vector2Int.left))) && !(CheckIfAnyIsStatus(currentCoordinate, solid) || CheckIfAnyIsStatus(nextCoordinate, solid)))
                                {
                                }
                                    //Debug.Log($"Case cx\n" +
                                    //          $"Case xn\n" +
                    ...
                    //ccnn
                    //ccnn
                    if ((currentCoordinateSlam[1] != open && currentCoordinateSlam[3] != open) ||
                        (nextCoordinateSlam[0] != open && currentCoordinateSlam[2] != open))
                        {
                            if ( (CheckForUnseen(currentCoordinate) ||  CheckForUnseen(nextCoordinate)) &&
                                  !(CheckIfAnyIsStatus(currentCoordinate, solid) || CheckIfAnyIsStatus(nextCoordinate, solid)))
                                {
                                    //Debug.Log($"Case cn\n" +
                                    //          $"Case cn\n" +
                                    //          $"Current coordinate: {currentCoordinateSlam[2]} {currentCoordinateSlam[3]}\n" +
                ...
                if (currentCoordinate.y < nextCoordinate.y) {
                    ...
                    if ((nextCoordinateSlam[1] != open || topRightSlam[0] != open ||
                        bottomLeftSlam[3] != open || currentCoordinateSlam[2] != open) &&
                        CheckForUnseen(currentCoordinate + Vector2Int.left) && CheckForUnseen(nextCoordinate + Vector2Int.right))
                        {
                            if ( (CheckForUnseen(currentCoordinate) ||  CheckForUnseen(nextCoordinate) || (CheckIfAllSlamStatusesSolid(nextCoordinate + Vector2Int.right) &&
                                  CheckIfAllSlamStatusesSolid(currentCoordinate + Vector2Int.left))) && !(CheckIfAnyIsStatus(currentCoordinate, solid) || CheckIfAnyIsStatus(nextCoordinate, solid)))
                                {
                    ...
                    if ((nextCoordinateSlam[3] != open || bottomRightSlam[2] != open ||
                        topLeftSlam[1] != open || currentCoordinateSlam[0] != open) &&
                        CheckForUnseen(currentCoordinate + Vector2Int.left) && CheckForUnseen(nextCoordinate + Vector2Int.right))
                        {
                            if ( (CheckForUnseen(currentCoordinate) ||  CheckForUnseen(nextCoordinate) || (CheckIfAllSlamStatusesSolid(nextCoordinate + Vector2Int.right) &&
                                  CheckIfAllSlamStatusesSolid(currentCoordinate + Vector2Int.left))) && !(CheckIfAnyIsStatus(currentCoordinate, solid) || CheckIfAnyIsStatus(nextCoordinate, solid)))
                    ...
                    //nncc
                    //nncc
                    if ((currentCoordinateSlam[0] != open && currentCoordinateSlam[2] != open) ||
                        (nextCoordinateSlam[1] != open && currentCoordinateSlam[3] != open))
                        {
                            if ( (CheckForUnseen(currentCoordinate) ||  CheckForUnseen(nextCoordinate)) &&
                                  !(CheckIfAnyIsStatus(currentCoordinate, solid) || CheckIfAnyIsStatus(nextCoordinate, solid)))
                                {
                ...
                if (currentCoordinate.y < nextCoordinate.y) {
                    ...
                    // cc
                    // cc
                    if ((currentCoordinateSlam[2] != open && currentCoordinateSlam[3] != open) ||
                        (nextCoordinateSlam[0] != open && currentCoordinateSlam[1] != open))
                        {
                            if ( (CheckForUnseen(currentCoordinate) ||  CheckForUnseen(nextCoordinate)) &&
                                  !(CheckIfAnyIsStatus(currentCoordinate, solid) || CheckIfAnyIsStatus(nextCoordinate, solid)))
                                {
                    ...
                    // nn
                    // nn
                    if ((currentCoordinateSlam[0] != open && currentCoordinateSlam[1] != open) ||
                        (nextCoordinateSlam[2] != open && currentCoordinateSlam[3] != open))
                        {
                            if ( (CheckForUnseen(currentCoordinate) ||  CheckForUnseen(nextCoordinate)) &&
                                  !(CheckIfAnyIsStatus(currentCoordinate, solid) || CheckIfAnyIsStatus(nextCoordinate, solid)))
                                {
